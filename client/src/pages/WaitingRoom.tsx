...
const WaitingRoom = () => {
  ...
  const isDeciding = session.status === "deciding";
  ...
  const getWaitingMessage = () => {
    ...
    if (!session.location) {
      return isHost
        ? "Choose where to search for food"
        : `${hostName} is picking a location...`;
    }
  ...
  return (
    <div className="min-h-screen bg-background p-4 pb-32">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500">{getWaitingMessage()}</p>

        <div
          className="bg-orange-100 text-primary font-bold py-3 px-6 rounded-full inline-flex items-center gap-2 mt-4 cursor-pointer"
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {session.participants.map((p) => (
          <div
            key={p.id}
            className="bg-white p-4 rounded-2xl shadow-sm flex flex-col items-center relative"
          >
            {p.isReady && (
              <CheckCircle
            ...
            <span
              className={`text-sm ${
                p.isReady ? "text-green-500" : "text-gray-400"
              }`}
            >
        ...
        <div
          className="border-2 border-dashed border-orange-200 rounded-2xl flex flex-col items-center justify-center p-4 cursor-pointer hover:bg-orange-50 min-h-[140px]"
          onClick={handleCopyCode}
        >
          <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center mb-2 shadow-sm text-primary font-bold text-xl">
            +
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] rounded-t-3xl">
        <div className="flex items-center justify-center gap-2 mb-4">
          ...
          <span className="font-bold">
            {readyCount}/{totalCount} friends are ready...
          </span>
        </div>
        ...
        {!myParticipant?.isReady ? (
          ...
          >
            <Utensils />{" "}
            {session.location ? "I'm Hungry" : `Waiting for ${hostName}...`}
          </button>
        ) : (
